...
$j(document).ready(function() {
	...
	if( $j.trim($j('#txtcust').val()) == "") { 
		...
		if( $j.trim($j('#txtpno').val()) != "") {
			...
			var response = geturl("getalerts_old_alert.php?customer=" + $j.trim(uid2[1]) + "&pno=" + $j.trim($j('#txtpno').val()) + "&rev=" + $j.trim($j('#txtrev').val()) + "&prevcust=" + $j.trim($j('#prevcust').val()) + "&prevpno=" + $j.trim($j('#prevpno').val()) + "&prevrev=" + $j.trim($j('#prevrev').val()) + "&ftype=quo&mode=edit"+"&quote_id="+quote_id); 
