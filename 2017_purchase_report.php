...
if($_REQUEST['start_date']==''){
	$sql = "SELECT * FROM porder_tb WHERE year(real_date) = 2017";
}
else if($_REQUEST['end_date']==''){
	$sql = "SELECT * FROM porder_tb WHERE year(real_date) = 2017 AND real_date >=  '".$fixed_start_date."'";
}
else{
	


	$sql = "SELECT * FROM porder_tb WHERE year(real_date) = 2017 AND real_date BETWEEN '".$fixed_start_date."' AND '".$fixed_end_date."'";
}

//echo $sql;

$fname = 'Purchase_Order_Report';
...
$sheet->setCellValue('A1', $_REQUEST['start_date']." to ".$_REQUEST['end_date']." Purchase Order Report");
			...
			switch($k) {
				...
				case 'vid':
					...
					while($row_vendor = mysql_fetch_assoc($res_vendor)){
						$val = strip_tags($row_vendor['c_shortname']);
					}
					break;
				case 'poid':
