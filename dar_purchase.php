...
if($_REQUEST['dupid'] != "") {
	...
	echo "<script type='text/javascript'>location.href='dar_purchase.php'</script>";
}
?>
...
function del(id) {
	...
	if(answer) 
		location.href = "dar_purchase.php?delid="+id;
	else 
		return false;
}
</script>
...
$sqs = "select * from porder_tb where cancel='0' AND rad='yes'";
$sqs.=" ORDER BY poid desc LIMIT $offset, $rowsPerPage ";

if( isset($_GET['hidden']) && $_GET['hidden'] == '1') {
	$sqs = "select * from porder_tb where cancel='0' AND rad='yes' and status='hide'";
	$sqs .= " ORDER BY poid desc LIMIT $offset, $rowsPerPage ";
}
if(isset($_REQUEST['btncus'])) {
	$srchc = $_REQUEST['srchc'];
	$sqs = "select * from porder_tb where cancel='0' AND rad='yes' and customer like '%".$srchc."%'";
	$sqs .= " ORDER BY poid desc LIMIT $offset, $rowsPerPage ";
}
	...
	$sqs="select * from porder_tb where cancel='0' AND rad='yes' and  part_no like '%".$pno."%'";
	$sqs.=" ORDER BY poid desc LIMIT $offset, $rowsPerPage ";
	//echo $sqs;
}

if($_REQUEST['srchv'] != "") {
	$sqs = "select * from porder_tb,vendor_tb where cancel='0' AND rad='yes' and  porder_tb.vid = vendor_tb.data_id and vendor_tb.c_name like '%".$_REQUEST['srchv']."%'";
...
$query = "select * from porder_tb where cancel='0' AND rad='yes'";
if($_REQUEST['srchv'] != "") {
	$query = "select * from porder_tb, vendor_tb where cancel='0'  AND rad='yes' and  porder_tb.vid = vendor_tb.data_id and vendor_tb.c_name like '%".$_REQUEST['srchv']."%'";
}
...
// print the navigation link 
	...
	<tr>
	  <td height="30" colspan="5"><font size="3"><strong>MANAGE Duplicate As Remake PURCHASE ORDERS</strong></font></td>
	  <td height="30" colspan="2" align="center"><a href="add_purchase.php">Add Purchase</a></td>
	  <td height="30" colspan="6"> <a href="manage_purchase.php">CLEAR SEARCH FILTERS</a></td>
	</tr>
	<tr>
	...
	<tr>
	  ...
	  <td width="107" align="center"><strong>Date</strong></td>
	...
	<tr>
		...
		?>
		  ...
		  <td align="center"><?php echo $rwc['canceldate'];?></td>
		  <td align="center"><?php 
