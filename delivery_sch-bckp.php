...
<?php
	if($st_date && $en_date){
		$crit = " and  ( unix_timestamp(str_to_date(substr(a.date1,locate('-',a.date1)+1),'%m-%d-%Y')) >=  unix_timestamp('".$st_date."')  
			and unix_timestamp(str_to_date(substr(a.date1,locate('-',a.date1)+1),'%m-%d-%Y')) <=  unix_timestamp('".$en_date."')  )
			";
	}
	else
	{
														...
														<table id="olist" cellpadding=5 cellspacing=0 width="100%" border=1 style="font-size:11px;">
															...
															<?php
																while($rws = mysql_fetch_array($ress)){
																    ...
																    $sql_pack = "SELECT COUNT(*) FROM packing_tb WHERE invoice_id='$invoice_id'";
															...
															?>
																<tr>
																	<td><a class="ttip_overlay_trig" href="javascript:void(0)"><?php echo $rws1['c_shortname'];?></a>
