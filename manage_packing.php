...
if(isset($_REQUEST['btncus'])) {
	...
	$sqs = "select pt.* from data_tb dt inner join packing_tb pt on dt.data_id = pt.customer where dt.c_shortname like '%".$srchc."%'";	
}
	...
	$sqs = "select * from packing_tb where part_no like '%".$pno."%'";
...
?>
	  <table class="manage" width="900" border="1" cellpadding="2" cellspacing="2" bordercolor="#e6e6e6" style="left:35px;">
		<tr>
		  ...
		  <td height="30" colspan="7" align="center"><a href="add_packing.php">ADD SLIP</a></td>
		</tr>
		<tr>
		...
		<tr>
		  ...
		  <td width="50" height="30" align="center"><strong>Logged</strong></td>
		</tr>
		<?php 
	  while($rwc = mysql_fetch_assoc($res1)) { ?>
		...
		?>
		  ...
		  <td>
		  	<input type="checkbox" name="" id="<?php echo $rwc['invoice_id']; ?>"  <?php echo ($rwc['loged']==1)? "checked=\"checked\"" : ""; ?>  value="<?php echo $rwc['invoice_id']; ?>" title="is loged" onclick="doAction1(this.value, this.checked);"/> 
		  </td>
	</tr>
	<?php 
	}
	...
	<script>
		...
		function doAction1(record_id, status) {		

			if(status == true){
				//alert('selected so should be checked here after');
				addCheckMark1(record_id,status);
			}
			else {
				//alert('de selected so should NOT be checked here after');

				jConfirm('Do you want to unmark the loged packingslip ?', 'Confirmation Dialog', function(answer) {
				if(answer) addCheckMark1(record_id, status);
				else document.getElementById(record_id).checked = true;
				});
			}
		}
		...
		function addCheckMark1(record_id,status) {
			...
			var url = "ajax_add_check_packingslip.php?frm=packing";
			url = url + "&record_id=" + record_id;
			url = url + "&status=" + status;			
			url = url + "&sid="+Math.random();
			xmlhttp.onreadystatechange = function(){stateChanged1(record_id)};;
			xmlhttp.open("post", url, true);
			xmlhttp.send(null);		
		}
		...
		function stateChanged1(record_id) {
			if (xmlhttp.readyState == 4) {
				if(xmlhttp.responseText==1){
					// alert('hi');
					window.location ="loged_packing_slip.php?invoice_id="+record_id;
				}
				document.getElementById("display_area").innerHTML = xmlhttp.responseText;  
			}
		}	
