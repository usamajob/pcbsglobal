...
<?php
if($_REQUEST['delid']!="") {
	$sqdel='delete from credit_tb where credit_id="'.$_REQUEST['delid'].'"';
	$resdel=mysql_query($sqdel) or die('error in data');
}

if($_REQUEST['dupid']!="") {

$sqdup1 = 'CREATE TEMPORARY TABLE tmptable_1 SELECT * FROM credit_tb WHERE credit_id= "'.$_REQUEST['dupid'].'"';
$sqdup2 = 'UPDATE  tmptable_1  SET credit_id= NULL';
$sqdup3 = 'INSERT INTO credit_tb  SELECT * FROM tmptable_1';
...
$qry_update_duplicate_invoice_date="UPDATE credit_tb  set podate='".date('m/d/Y')."', credit_date=now() Where credit_id=".$duplicate_invoice_id;
mysql_query($qry_update_duplicate_invoice_date);
///////end new-code

$sqdup4 = mysql_query($sqdup4) or die('error in data 4');

$sqsc = "select * from credit_items_tb where pid='".$_REQUEST['dupid']."'";
$resc = mysql_query($sqsc) or die('error in data');

while($rwsc=mysql_fetch_array($resc)) {
	$sqin = "insert into credit_items_tb(item,itemdesc,qty2,uprice,tprice,pid) values('".$rwsc['item']."','".$rwsc['itemdesc']."','".$rwsc['qty2']."','".$rwsc['uprice']."','".$rwsc['tprice']."','".$duplicate_invoice_id."')";
	$resin = mysql_query($sqin) or die('error in q');
}

echo "<script language='javascript'>location.href='manage_credit.php'</script>";
}
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<title>Manage Credit Offset</title>
...
if(answer) {
	location.href="manage_credit.php?delid="+id;
}
else {
	return false;
}
...
$sqs = "select * from credit_tb";
$sqs .= " ORDER BY credit_id desc LIMIT $offset, $rowsPerPage ";


	if(isset($_REQUEST['btncus'])) {
		$srchc =$_REQUEST['srchc'];
		$sqs="select * from credit_tb where customer like '%".$srchc."%'";
		$sqs.=" ORDER BY credit_id desc LIMIT $offset, $rowsPerPage ";
	}
		...
		$sqs="select * from credit_tb where part_no like '%".$pno."%'  and customer like '%".$cname."%'";
		$sqs.=" ORDER BY credit_id desc LIMIT $offset, $rowsPerPage ";
		/*$ponm =$_REQUEST['srch']-9976;

		$sqs="select * from invoice_tb where invoice_id ='".$ponm."'";
...
?>
	  <table class="manage" width="900" border="1" cellpadding="2" cellspacing="2" bordercolor="#e6e6e6" style="left:35px;">
		<tr>
		  <td height="30" colspan="6"><font size="3"><strong>MANAGE Credit Offset</strong></font></td>
		  <td height="30" colspan="5" align="center"><a href="add_credit.php">ADD Credit Offset</a></td>
		</tr>
		<tr>
		  <td height="30" colspan="11" align="center"><?php echo $first . $prev . $nav . $next . $last; ?></td>
		</tr>
		<tr>
		  <td width="56" height="30" align="center"><strong>Credit ID </strong></td>
		  <td width="56" height="30" align="center"><strong>Credit # </strong></td>
		  ...
		  <td width="107" align="center"><strong>Credit DATE</strong></td>
	  ...
	  ?>
		<tr>
		  <td height="30" align="center"><?php echo $rwc['credit_id'];?>&nbsp;</td>
		  <td height="30" align="center"><?php echo $rwc['credit_id']+10098;?>&nbsp;</td>
		  ...
		  <td align="center"><?php
		  $arr1=explode("-",$rwc['credit_date']);
		  $dtt=$arr1[1]."/".$arr1[2]."/".$arr1[0];
		   echo $dtt;
		   
		   ?></td>
		  <td height="30" align="center"><a href="edit_credit.php?id=<?php echo $rwc['credit_id'];?>">Edit</a></td>
		  <td height="30" align="center"><a href="downloadc-pdf2.php?id=<?php echo $rwc['credit_id'];?>">Download Pdf</a></td>
		  <td height="30" align="center"><a target="_blank" href="downloadc-pdf2.php?id=<?php echo $rwc['credit_id'];?>&amp;oper=view">VIEW Pdf</a></td>
		  <td height="30" align="center"><a href="#" onclick="del('<?php echo $rwc['credit_id'];?>');">Delete</a></td>
		  <td height="30" align="center"><a href="manage_credit.php?dupid=<?php echo $rwc['credit_id'];?>">Duplicate</a></td>
		</tr>
		<?php 
	  }
	  ?>
...
?>
	  ...
	  <table width="600" border="1" align="center" cellpadding="2" cellspacing="2" bordercolor="#e6e6e6">
		<tr>
		  <td height="50"><strong>Credit Offset is not found</strong></td>
		</tr>
	  </table>
	  <p>
		<?php 
