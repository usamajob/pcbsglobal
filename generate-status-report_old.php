...
$sql = "select distinct p.allow ,p.po, p.poid, p.customer, p.part_no, p.rev, p.date1, p.dweek,p.note,p.cus_due,p.supli_due, i.invoice_id , v.c_shortname vc
from porder_tb p
LEFT JOIN invoice_tb i on
(p.part_no = i.part_no AND p.rev = i.rev AND p.po = i.po)
LEFT JOIN vendor_tb v on v.data_id = p.vid
where ".$wherestr."  order by v.c_shortname,p.dweek";
