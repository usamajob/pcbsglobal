...
if(isset($_POST['Submit'])) {
	...
	$sqin = "insert into invoice_tb (vid, sid, namereq, svia, fcharge, city, state, sterm, comments, podate, customer, part_no, rev, delto, ord_by, date1, dweek, po, our_ord_num, saletax, no_layer, sp_reqs, svia_oth, salesrep, commision, comval) values('".$_REQUEST['vid']."','".$_REQUEST['sid']."', '".escpe($_REQUEST['namereq'])."', '".$_REQUEST['svia']."', '".$_REQUEST['fcharge']."', '".escpe($_REQUEST['city'])."', '".escpe($_REQUEST['state'])."', '".$_REQUEST['sterms']."', '".escpe($_REQUEST['area1'])."', '".date('m/d/Y')."', '".escpe($_REQUEST['customer'])."', '".$_REQUEST['part_no']."', '".$_REQUEST['rev']."', '".$_REQUEST['delto']."', '".$_REQUEST['ord_by']."', '".$_REQUEST['date1']."', '".$dweek."', '".$_REQUEST['po']."', '".$_REQUEST['oo']."', '".$_REQUEST['stax']."', '".$_REQUEST['lyrcnt']."', '".escpe($_REQUEST['specialreqval'])."', '".$_POST['svia_oth']."',  '".escpe($_POST['salesrep'])."', '".$_POST['commision']."', '".$_POST['totcomm']."')";
...
<tr>
	...
	<td height="30">
	<select name="vid" id="vid" onchange="getsalesrep()">
...
<div id='reqDiv' style='z-index: 110; padding: 10px; background: #eee; border: 1px solid #369; position: absolute; top:1200px; left: 250px; display: none;' onmousedown='mydragg.startMoving(this,"container",event);' onmouseup='mydragg.stopMoving("container");'></div>
<div id='alertDiv' style='z-index: 100; padding: 10px; background: #c1fff8; border: 1px solid #369; position: absolute; top:1100px; left: 250px; display: none;' onmousedown='mydragg.startMoving(this,"container",event);' onmouseup='mydragg.stopMoving("container");'></div>
