...
if(isset($_POST['Submit'])) {
	...
	//Updating alerts with new part_no and rev
	if( trim($_POST['prevpno']) != trim($_POST['txtpno']) ) { 
		//|| trim($_POST['prevrev']) != trim($_POST['txtrev'])
		$asql = "update alerts_tb set part_no='".$_POST['txtpno']."' where part_no='".$_POST['prevpno']."'";
		//, rev='".$_POST['txtrev']."' //update
		// and rev= '".$_POST['prevrev']."' //where
	...
	$admcmntstat = isset($_REQUEST['admcmntstat']) ? $_REQUEST['admcmntstat'] : 'no';
	...
	*/
	...
	$fingers_gold = isset($_POST['chk25']) ? $_POST['chk25'] : 'No';
	...
	$con_impe1 = isset($_POST['chk35']) ? $_POST['chk35'] : 'No';
	...
	$blind = isset($_POST['chk37']) ? $_POST['chk37'] : 'No';

	$buried = isset($_POST['chk39']) ? $_POST['chk39'] : 'No';
	...
	$hdi_design = isset($_POST['chk200']) ? $_POST['chk200'] : 'No';

	$cond_vias = isset($_POST['chk210']) ? $_POST['chk210'] : 'No';

	$resin_filled = isset($_POST['chk215']) ? $_POST['chk215'] : 'No';
	...
	$route = isset($_POST['chk61']) ? $_POST['chk61'] : 'No';
	...
	$array = isset($_POST['chk63']) ? $_POST['chk63'] : 'NO';
	...
	$array_design = isset($_POST['chk65']) ? $_POST['chk65'] : 'No';

	$design_array = isset($_POST['chk67']) ? $_POST['chk67'] : 'No';
	...
	$bevel = isset($_POST['chk75']) ? $_POST['chk75'] : 'No';

	$counter_sink = isset($_POST['chk77']) ? $_POST['chk77'] : 'No';

	$cut_outs = isset($_POST['chk79']) ? $_POST['chk79'] : 'No';

	$slots = isset($_POST['chk81']) ? $_POST['chk81'] : 'No';
	...
	$mark = isset($_POST['chk85']) ? $_POST['chk85'] : 'No';
	...
	$micro_s = isset($_POST['chk90']) ? $_POST['chk90'] : 'NO';

	$test_stamp = isset($_POST['chk92']) ? $_POST['chk92'] : 'No';
	...
	$xouts = isset($_POST['chk95']) ? $_POST['chk95'] : 'no';

	if(strtolower($xouts) == 'yes' && $_POST['textfield28'] != "") {
		$xouts1 = $_POST['textfield28'];
	}

	$rosh = isset($_POST['chk97']) ? $_POST['chk97'] : 'No';
...
if ($rest != '-C') {
	...
	if ($_REQUEST['cancharge'] == 'yes') {
		...
		//The id of the product you want to copy
...
$sqin = "update order_tb set cust_name='".$txtcust."',part_no='".$_REQUEST['txtpno']."',rev='".$_REQUEST['txtrev']."',req_by='".$txtreq."',
	...
	lead_times='".$_REQUEST['txtlead']."', quote_by='".$_REQUEST['txtquote']."', special_inst='".$_REQUEST['txtinst']."', special_instadmin='".$_REQUEST['txtinstadmin']."', is_spinsadmact='".$admcmntstat."',
	...
	con_impe1='".$con_impe1. "',new_or_rep='".$_REQUEST['nor']. "',necharge='".$_REQUEST['necharge']. "',descharge='".$_REQUEST['descharge']. "',cpo='".$_REQUEST['cpo']. "',opo='".$_REQUEST['opo']	. "',pr11='".$_REQUEST['pr11']. "',pr12='".$_REQUEST['pr12']. "',pr13='".$_REQUEST['pr13']. "',pr14='".$_REQUEST['pr14']. "',pr15='".$_REQUEST['pr15']
...
function test() {
	...
	var showlink = geturl("getRequirements.php?cid="+uid3);
...
$j(document).ready(function() {
	...
	$j("#Submit").click(function() {
		if( $j.trim($j('#txtpno').val()) != "") {
			//&& $j.trim($j('#txtrev').val()) != ""					
			$j('#alertDiv').html(geturl("getalerts.php?pno=" + $j.trim($j('#prevpno').val()) + "&ftype=edit")); 
			//+ "&rev=" + $j.trim($j('#prevrev').val())	
			$j('#alertDiv').show();		
		...
		if(flag1 == 1) { // Customer profile popup			
			...
			/* //Commented cust profile temporarily
			$j('#reqDiv').show(); 
			if(flag3 <= 1) flag3 = flag3 + 1; */
...
<tbody><tr>
	...
	<tr>             
		...
		<!-- <tr>
			<td height="25" colspan="3"><strong>LEAD TIMES : 7 DAYS : $2 (base price) || <br>
   ...
   <tr>
       ...
       <!-- <label> <input type="text" name="txtcust" id="txtcust"  value="<?php // echo $rws['cust_name'];?>"/>
...
testsaved(reqbysaved);
      ...
      <td height="25" colspan="2" id="content2"><strong>Email :</strong>
...
-->
    ...
    <tr>
      <td height="25" colspan="3"><strong>FAX :</strong>
...
<div id='complexform' <?php echo ($rws['simplequote'] == '1' ? "style='display:none'" : '') ?> >
	...
	<tr><td>
		...
		<input type="checkbox" name="cancharge" <?php if ($rws['cancharge'] == 'yes') echo 'checked'; ?> value="yes">
...
<tr>
	...
	<td>
		<table width='100%'>
		<tr>
			<td height="25" colspan='2' bgcolor="#336699" align="Left">
		...
		<tr>
			...
			<strong>Replace with Comments</strong> <input type="checkbox" name="admcmntstat" value="yes" <?php if($rws['is_spinsadmact'] == 'yes') { ?> checked <?php } ?> ><!-- <br>
			Yes 
			<input name="admcmntstat" type="radio" id="admcmntstat1" value="yes" <?php if($rws['is_spinsadmact'] == 'yes') { ?> checked <?php } ?> />
			<br>
			No &nbsp;<input name="admcmntstat" type="radio" id="admcmntstat2" value="no" <?php if($rws['is_spinsadmact'] == 'no'){ ?> checked <?php } ?> /> -->
			</td>
		</tr>
		</table>
	</td>	
...
<tr>
	<td height="25" width="307" bgcolor="#336699"><font color="white"><strong>Number of Layers : </strong></td>
	<td height="25" bgcolor="#336699"><font color="white"><strong>Material Required : </strong></td>
	...
	2 <input name="chki2" type="checkbox" id="chki2" value="2" <?php if($rws['ipc_class']=='2'){ ?> checked <?php }?> />
	3 <input name="chki3" type="checkbox" id="chki3" value="3" <?php if($rws['ipc_class']=='3'){ ?> checked <?php }?> /></td>
</tr>

<tr>
	<td height="25"> Single Sided <input name="chk1" type="radio" id="chk1" value="single" <?php if($rws['no_layer']=='single'){ ?> checked <?php }?> />
	...
	<br />     </td>
	<td height="25"> FR-4 <input name="chk7" type="radio" id="chk7" value="FR-4"   <?php if($rws['m_require']=='FR-4'){ ?> checked <?php }?>/>

	FR-4/170TG Min <input name="chk7" type="radio" id="chk8" value="FR-4/170TG Min"  <?php if($rws['m_require']=='FR-4/170TG Min'){ ?> checked <?php }?>/>

	<br /><br />Rogers 4003 <input name="chk7" type="radio" id="chk9" value="Rogers 4003"   <?php if($rws['m_require']=='Rogers 4003'){ ?> checked <?php }?> />
	...
	<br /></td>
	<td height="25" valign="top"><strong>Thickness:</strong> 0.031&quot; <input name="chk13" type="radio" id="chk192" value="0.031" <?php if($rws['thickness']=='0.031'){ ?> checked <?php }?>  />

	0.062&quot; <input name="chk13" type="radio" id="chk14" value="0.062"  <?php if($rws['thickness']=='0.062'){ ?> checked <?php }?>  />

	<br /><br />0.093&quot; <input name="chk13" type="radio" id="chk15" value="0.093" <?php if($rws['thickness']=='0.093'){ ?> checked <?php }?>  />
	...
	<br />
	<br /> +-10% <input onChange="thktolclr()" onSelect="thktolclr()" name="chk17" type="radio" id="chk17" value="+/- 10%"  <?php if($rws['thickness_tole']=='+/- 10%'){ ?> checked <?php }?> />

	Other <input name="chk17" type="radio" id="chk101" value="Other" <?php if($rws['thickness_tole']!='+/- 10%'){ ?> checked <?php }?> />

	<input name="txtother5" type="text" id="txtother5" size="10" <?php if($rws['thickness_tole']!='+/- 10%'){ ?> value="<?php echo $rws['thickness_tole']; ?>" <?php }?>  />

	<br />
	<br /> Adder %: <input name="per3" type="text" id="txtother18" value="<?php echo $rws['per3']; ?>" /></td>
</tr>
...
<tr>
	<td height="25"><strong>External Cu Oz:</strong> 0.5 <input onChange="strtcu()" onSelect="strtcu()" name="chk10" type="radio" id="chk10" value="0.5"  <?php if($rws['start_cu']=='0.5'){ ?> checked <?php } ?>   />
	...
	Other : <input name="chk10" type="radio" id="chk100" value="Other" <?php if(($rws['start_cu']!='0.5')and($rws['start_cu']!='1')and($rws['start_cu']!='2')){ ?> checked <?php }?> /> <input type="text" name="txtother3" id="txtother3" <?php if(($rws['start_cu']!='0.5')and($rws['start_cu']!='1')and($rws['start_cu']!='2')){ ?> value="<?php echo $rws['start_cu']; ?>" <?php }?> />

	<br /><br />Adder % : <input type="text"  id="txtother191" name="txtother191" value="<?php echo $rws['pad1']; ?>"   />

	<br /><br /><strong>Plated Cu in Holes (Min.):</strong> .0010 <input onChange="pltdcuhole()" onSelect="pltdcuhole()" name="chk22" type="radio" id="chk22" value=".0010"  <?php if($rws['plated_cu']=='.0010'){ ?> checked <?php }?>  />
	...
	<br /><br /><strong>Fingers Nickel/Hard Gold: </strong><input type="checkbox" name="chk25" value="yes" <?php if($rws['fingers_gold']=='yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk25" type="radio" id="chk25" value="yes" <?php if($rws['fingers_gold']=='yes'){ ?> checked <?php } ?> />
	NO
	...
	<br /></td>
	<td height="25" valign="top"><strong>Trace Min. = </strong>
	.006 <input name="chk27" type="radio" id="chk27" value=".006" <?php if($rws['trace_min']=='.006'){ ?> checked <?php } ?> /> 
	.005 <input name="chk27" type="radio" id="chk28" value=".005" <?php if($rws['trace_min']=='.005'){ ?> checked <?php } ?> /> 
	.004 <input name="chk27" type="radio" id="chk29" value=".004" <?php if($rws['trace_min']=='.004'){ ?> checked <?php } ?> />
	.003 <input name="chk27" type="radio" id="chk30" value=".003" <?php if($rws['trace_min']=='.003'){ ?> checked <?php } ?> />
	<br />
	Other 
	<input name="chk27" type="radio" id="chk212" value="Other" <?php if($rws['trace_min'] != '.003' && $rws['trace_min'] != '.004' && $rws['trace_min'] != '.005' && $rws['trace_min'] != '.005' && $rws['trace_min'] != '') { ?> checked <?php } ?> />
	<input name="txtother54" type="text" id="txtother54" size="10" <?php if($rws['trace_min'] != '.003' && $rws['trace_min'] != '.004' && $rws['trace_min'] != '.005' && $rws['trace_min'] != '.005' && $rws['trace_min'] != 'Other') { echo 'value="'.$rws['trace_min'].'"'; } ?> />

	<br /><br /><strong>Space Min. =</strong>
	.006 <input name="chk31" type="radio" id="chk31" value=".006"  <?php if($rws['space_min']=='.006'){ ?> checked <?php }?>/>

	.005 <input name="chk31" type="radio" id="chk32" value=".005" <?php if($rws['space_min']=='.005'){ ?> checked <?php }?> />

	.004 <input name="chk31" type="radio" id="chk33" value=".004" <?php if($rws['space_min']=='.004'){ ?> checked <?php }?>/>

	.003 <input name="chk31" type="radio" id="chk34" value=".003" <?php if($rws['space_min']=='.003'){ ?> checked <?php }?> />
	<br /> 
	Other 
	<input name="chk31" type="radio" id="chk213" value="Other" <?php if($rws['space_min'] != '.003' && $rws['space_min'] != '.004' && $rws['space_min'] != '.005' && $rws['space_min'] != '.005' && $rws['space_min'] != '') { ?> checked <?php } ?> />
	<input name="txtother55" type="text" id="txtother55" size="10" <?php if($rws['space_min'] != '.003' && $rws['space_min'] != '.004' && $rws['space_min'] != '.005' && $rws['space_min'] != '.005' && $rws['space_min'] != 'Other') { echo 'value="'.$rws['space_min'].'"'; } ?> />
	<br /><br /><strong>Controlled Impedance:</strong>
	<input type="checkbox" name="chk35" value="Yes"  <?php if($rws['con_impe1']=='Yes'){ ?> checked <?php } ?> ><!-- Yes <input name="chk35" type="radio" id="chk35" value="Yes"  <?php if($rws['con_impe1']=='Yes'){ ?> checked <?php }?>/>
	...
	<td height="25" valign="top"><strong>Smallest  Hole Size:</strong> <input name="txtother8" type="text" id="txtother8" size="8"   value="<?php echo $rws['hole_size'];?>"/>

	<br /><strong>

	Smallest Pad :</strong> <input name="txtother14" type="text" id="txtother14" size="10"  value="<?php echo $rws['pad'];?>"/>

	<br />   <br /><strong>Blind Vias:</strong> <input type="checkbox" name="chk37" value="yes" <?php if($rws['blind']=='yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk37" type="radio" id="chk37" value="yes" <?php if($rws['blind']=='yes'){ ?> checked <?php }?>  />
	...
	<br /><strong>Buried Vias: </strong><input type="checkbox" name="chk39" value="yes"  <?php if($rws['buried']=='yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk39" type="radio" id="chk39" value="yes"  <?php if($rws['buried']=='yes'){ ?> checked <?php }?>/>

	No <input name="chk39" type="radio" id="chk40" value="No" <?php if($rws['buried']=='No'){ ?> checked <?php }?> /> -->

	<br /><br />
	<strong>HDI Design:</strong>	<input type="checkbox" name="chk200" value="Yes" <?php if($rws['hdi_design']=='Yes'){ ?> checked <?php } ?> >   
	<!-- Yes <input name="chk200" type="radio" id="chk200" value="Yes"  <?php if($rws['hdi_design']=='Yes'){ ?> checked <?php }?>/>

	No <input name="chk200" type="radio" id="chk201" value="No" <?php if($rws['hdi_design']=='No'){ ?> checked <?php }?> /> -->
	<br /><br />
	<strong>Non-Conductive Filled/Plated Over:</strong>&nbsp;<input type="checkbox" name="chk215" value="Yes" <?php if($rws['resin_filled']=='Yes'){ ?> checked <?php } ?> >
	<!-- <br> Yes <input name="chk215" type="radio" id="chk215" value="Yes"  <?php if($rws['resin_filled']=='Yes'){ ?> checked <?php }?>/>
	No <input name="chk215" type="radio" id="chk216" value="No" <?php if($rws['resin_filled']=='No'){ ?> checked <?php } ?> /> -->

	<br />
	<strong>Conductive Filled Vias:</strong>&nbsp;<input type="checkbox" name="chk210" value="Yes" <?php if($rws['cond_vias']=='Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk210" type="radio" id="chk210" value="Yes"  <?php if($rws['cond_vias']=='Yes'){ ?> checked <?php }?>/>
...
<tr>
	<td height="25"><strong>Finish:</strong> 
	...
	<td height="25"><strong>Mask Sides:</strong> N/A <input onSelect="msidenot();" onChange="msidenot();" name="chk48" type="radio" id="chk48" value="N/A"  <?php if($rws['mask_size']=='N/A'){ ?> checked <?php }?>  />
	...
	<td height="25"><strong>S/S Sides:</strong>N/A <input name="chk55" onChange="nomennot();" onSelect="nomennot();" type="radio" id="chk55" value="N/A"  <?php if($rws['ss_side']=='N/A'){ ?> checked <?php }?>/>
...
<tr>
	...
	<td height="25"><strong>Individual Route 1-up: </strong>
	<input type="checkbox" name="chk61" value="Yes" <?php if($rws['route']=='Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk61" type="radio" id="chk61" value="Yes" <?php if($rws['route']=='Yes'){ ?> checked <?php }?> />

	No <input name="chk61" type="radio" id="chk62" value="No"  <?php if($rws['route']=='No'){ ?> checked <?php }?>/> -->

	<br /><br /><strong>Board Size (in.)</strong> <input name="txtother12" type="text" id="txtother12" size="5"  value="<?php echo $rws['board_size1'];?>" />

	X <input name="txtother13" type="text" id="txtother13" value="<?php echo $rws['board_size2'];?>" size="5" />

	<br /><br /><strong>Array:</strong> <input type="checkbox" name="chk63" value="YES" <?php if($rws['array']=='YES'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk63" type="radio" id="chk63" value="YES"   <?php if($rws['array']=='YES'){ ?> checked <?php }?> />
	No <input name="chk63" type="radio" id="chk64" value="NO"  <?php if($rws['array']=='NO'){ ?> checked <?php }?> />  -->

	<br /><strong>Bds Per Array :</strong> <input name="txtother26" type="text" id="textfield26" size="4"    value="<?php echo $rws['b_per_array'];?>"/>

	<br /><strong>Array Size: </strong> <input name="txtother15" type="text" id="txtother15" size="5" value="<?php echo $rws['array_size1'];?>" /> X
	...
	+-.005 <input onChange="routtol()" onSelect="routtol()"  name="chk204" type="radio" id="chk204" value="+/-.005" <?php if($rws['route_tole'] == '+/-.005') { ?> checked <?php } ?> />
	...
	<td height="25"><strong>Array Design Provided:</strong> <input type="checkbox" name="chk65" value="Yes" <?php if($rws['array_design'] == 'Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk65" type="radio" id="chk65" value="Yes"   <?php if($rws['array_design'] == 'Yes'){ ?> checked <?php }?> />
	No <input name="chk65" type="radio" id="chk66" value="No" <?php if($rws['array_design'] == 'No'){ ?> checked <?php }?> /> -->

	<br /><strong><br />
	Factory to Design Array: </strong> <input type="checkbox" name="chk67" value="yes" <?php if($rws['design_array']== 'yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk67" type="radio" id="chk67" value="yes" <?php if($rws['design_array']== 'yes'){ ?> checked <?php }?>/>
	...
	<td height="25"><strong>Milling: </strong> <input type="checkbox" name="chk75" value="yes" <?php if($rws['bevel']=='yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk75" type="radio" id="chk75" value="yes" <?php if($rws['bevel']=='yes'){ ?> checked <?php }?> />
	No <input name="chk75" type="radio" id="chk76" value="No" <?php if($rws['bevel']=='No'){ ?> checked <?php }?> /> -->

	<br /><br /><strong>Countersink:</strong> <input type="checkbox" name="chk77" value="yes" <?php if($rws['counter_sink']=='yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk77" type="radio" id="chk77" value="yes" <?php if($rws['counter_sink']=='yes'){ ?> checked <?php }?> />
	No <input name="chk77" type="radio" id="chk78" value="No"  <?php if($rws['counter_sink']=='No'){ ?> checked <?php }?>/> -->

	<br /><br /><strong>Control Depth:</strong> <input type="checkbox" name="chk79" value="Yes" <?php if($rws['cut_outs']=='Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk79" type="radio" id="chk79" value="Yes" <?php if($rws['cut_outs']=='Yes'){ ?> checked <?php }?> />
	No <input name="chk79" type="radio" id="chk80" value="No"   <?php if($rws['cut_outs']=='No'){ ?> checked <?php }?>/> -->

	<br /><br /><strong>Edge Plating:</strong> <input type="checkbox" name="chk81" value="Yes" <?php if($rws['slots']=='Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk81" type="radio" id="chk81" value="Yes"   <?php if($rws['slots']=='Yes'){ ?> checked <?php }?>/>
...
<tr>
	<td height="25"><strong>Logo:</strong>
	None <input name="chk83" type="radio" id="chk83" value="None"  <?php if($rws['logo']=='None'){ ?> checked <?php } ?> />
	...
	<input name="txtother56" type="text" id="txtother56" size="5" <?php if($rws['logo'] != 'Factory' && $rws['logo'] != 'None' && $rws['logo'] != 'Other') { ?> value="<?php echo $rws['logo'] ?>" <?php } ?> />

	<br /><br /><strong>94V-0 Mark: </strong> <input type="checkbox" name="chk85" value="Yes" <?php if($rws['mark']=='Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk85" type="radio" id="chk85" value="Yes"  <?php if($rws['mark']=='Yes'){ ?> checked <?php }?> />
	...
	<br /> <br />YYWW <input onChange="clrother17()" onSelect="clrother17()" name="chk87" type="radio" id="chk89" value="YYWW"   <?php if($rws['date_code']=='YYWW'){ ?> checked <?php }?>/>
	...
	<br /></td>
	<td height="25"><strong>Microsection Required: </strong> <input type="checkbox" name="chk90" value="YES" <?php if($rws['micro_section']=='YES'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk90" type="radio" id="chk90" value="YES"   <?php if($rws['micro_section']=='YES'){ ?> checked <?php }?>/>
	No <input name="chk90" type="radio" id="chk91" value="NO"  <?php if($rws['micro_section']=='NO'){ ?> checked <?php }?> /> -->

	<br /><br /><strong>Electrical Test Stamp: </strong> <input type="checkbox" name="chk92" value="Yes"  <?php if($rws['test_stamp']=='Yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk92" type="radio" id="chk92" value="Yes"  <?php if($rws['test_stamp']=='Yes'){ ?> checked <?php }?> />
	...
	<td height="25"><strong>X-Outs Allowed:</strong> <input type="checkbox" name="chk95" value="yes" <?php if($rws['xouts']=='yes'){ ?> checked <?php } ?> >
	<!-- Yes <input name="chk95" type="radio" id="chk95" value="yes"   <?php if($rws['xouts']=='yes'){ ?> checked <?php }?> />
	...
	<br /><strong>RoHS Cert: </strong> <input type="checkbox" name="chk97" value="Yes" <?php if(strtolower($rws['rosh_cert'])==strtolower('Yes')){ ?> checked <?php }?> >
	<!-- Yes <input name="chk97" type="radio" id="chk97" value="Yes" <?php if(strtolower($rws['rosh_cert'])==strtolower('Yes')){ ?> checked <?php }?> />
...
<table class="upTT" width="880" border="1" style="border-color: #336699; left:35px;" cellpadding="1" cellspacing="0" bordercolor="#e6e6e6">
<tr>
	<td height="25" colspan="3"><div style="position: relative">
	...
	</div><div id='specialreq' style='color: #000; font: 11px/25px Verdana;'><?php echo ($rws['sp_reqs'] != "" ? "<strong>Special requirements:</strong> ". $rws['sp_reqs'] : ""); ?></div></td>
</tr>
