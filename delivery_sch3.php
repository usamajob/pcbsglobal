                                                ...
                                                <table align="left" cellpadding="5" cellspacing="1" width="100%" style="position: relative; top: -20px; margin-left:0px">
                                                    <tbody>
                                                        ...
                                                        $sqs = "select  b.poid,
                                                                    ...
                                                                    from  porder_tb b 
                                                                    LEFT  OUTER JOIN invoice_tb c  on (b.part_no = c.part_no and b.rev = c.rev)
...
# An odd row
             ...
             //   $sql_pack = "SELECT COUNT(*) as cnt ,p.date1,p.svia,p.delto,p.po FROM porder_tb o,packing_tb p WHERE p.rev='$rev' and p.part_no='".$rws['part_no']."' ";
			 $sql_pack = "SELECT COUNT(*) as cnt ,p.date1,p.svia,p.delto,p.po FROM porder_tb o,packing_tb p WHERE p.part_no=o.part_no and p.part_no='".$rws['part_no']."' and p.rev='$rev' ";
				//'$pno'
                $res_pack = mysql_query($sql_pack) or die(mysql_error());
                $row_pack = mysql_fetch_row($res_pack);
//echo $row_pack[0];
                $test = $row_pack[0];
                $shipvia = $row_pack['svia'];
                $delto = $row_pack['delto'];
