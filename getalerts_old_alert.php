...
$query = "select * from alerts_tb where ( ( customer='".$txtcust."' and part_no='".$_GET['pno']."' and rev='".$_GET['rev']."') or (refid='".$_SESSION['refid']."' and refid <> '') ) and atype='p' and instr(viewable, '".$_REQUEST['ftype']."') or (quote_id = ".$_GET['quote_id'].") > 0 and trim(alert) <> '' order by frm, id";

//$query = "select * from alerts_tb where (( quote_id = ".$_GET['quote_id'].") or (refid='".$_SESSION['refid']."' and refid <> '') ) and atype='p' and instr(viewable, '".$_REQUEST['ftype']."') > 0 and trim(alert) <> '' order by frm, id";
...
var quote_id = "<?php echo isset($_GET['quote_id']) ? $_GET['quote_id'] : '12345' ?>";
...
$(document).ready(function() {
	...
	$('#addalert').click(function(e) {
		if($.trim($('#txtalert').val()) == '') alert('Add alert text');
			...
			$.get("updatealerts.php", { atext: $.trim($('#txtalert').val()), utype: 'add', prevcust: cust, prevpno: pno, prevrev: rev, ftype: ftype,quote_id:quote_id }, function() {
